...
class Announcement extends CI_Controller {
	...
	public function index() {
        ...
        $page_data['data'] = $this->db->select('announcement.*, language.name as language_name')->from('announcement')->join('language', 'announcement.language_id = language.id', 'left')->order_by('id','desc')->get()->result_array();
	...
	public function create(){
        ...
        if(!empty($_FILES['document']['name'])) {
            ...
            if(isset($insert)){
                $message = array('message'=>"Announcement Created Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Create Announcement", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
	...
	public function edit($param=''){
        ...
        $page_data['page_name'] = 'announcement';
        $page_data['languages'] = $languages;
        $page_data['row_data']  =   $this->db->get_where('announcement',array('id'=>$param))->row_array();
        $page_data['data'] = $this->db->order_by('id','desc')->get('announcement')->result_array();
        $this->load->view('admin/common', $page_data);
	}
	public function update($param=''){
	   ...
	   $this->form_validation->set_rules('document', 'document');
        ...
        if($this->form_validation->run() == false){
            ...
            if(!empty($_FILES['document']['name'])) {
                $row_data  =  $this->db->get_where('announcement',array('id'=>$param))->row('document');
        	    if(!empty($row_data)) {
	        ...
	        $update = $this->db->update('announcement',$data);
	        if(isset($update)){
                $message = array('message'=>"Announcement Updated Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Update Announcement", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/announcement'),'refresh');
    ...
    public function delete($param='')
	{
        ...
        $delete = $this->db->where('id', $param)->delete('announcement');
        
        if(isset($delete)){
            $message = array('message'=>"Announcement Delete Successfully", 'class'=>'success');
        }else{
            $message = array('message'=>"Failed to Delete Announcement", 'class'=>'danger');
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/announcement'),'refresh');
	}
	...
	public function fetch_data() {
        ...
        $main_table = 'announcement'; // Main table
        $controller_name = "admin/Announcement"; // Controller name
        ...
        foreach ($data as $row) {
            ...
            $row_data[] = $row['type'];
            $row_data[] = '<a href="' . base_url('upload/' . $row['document']) . '" target="_blank"><i class="fa fa-eye" aria-hidden="true"></i></a>';
