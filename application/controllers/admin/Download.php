...
class Download extends CI_Controller {
	...
	public function index() {
        ...
        $page_data['download_category'] = $this->db->get('download_category')->result_array();
        $page_data['data'] = $this->db->get('download')->result_array();
    ...
    $this->form_validation->set_rules('download_category_id', 'Download Category', 'trim|required');
    if($this->form_validation->run() == false){
        $message = array('message'=>(validation_errors()), 'class'=>'danger');
        redirect(site_url('admin/download'),'refresh');
    }else{
        ...
        $data['download_category_id'] = $this->security->xss_clean($this->input->post('download_category_id'));
     if(!empty($_FILES['pdf']['name'])) {
        ...
        $insert = $this->db->insert('download',$data);          
        if(isset($insert)){
            $message = array('message'=>"Download Created Successfully", 'class'=>'success');
        }else{
            $message = array('message'=>"Failed to Create Download", 'class'=>'danger');
        }
    }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/download'),'refresh');
        ...
        $page_data['page_name'] = 'download';
        $page_data['download_category'] = $this->db->get('download_category')->result_array();
        $page_data['row_data']  =   $this->db->get_where('download',array('id'=>$param))->row_array();
        $page_data['data'] = $this->db->get('download')->result_array();
        $this->load->view('admin/common', $page_data);
	}
	public function update($param=''){
        ...
        if($this->form_validation->run() == false){
            ...
            $data['download_category_id'] = $this->security->xss_clean($this->input->post('download_category_id'));
            if(!empty($_FILES['pdf']['name'])) {
                $row_data  =  $this->db->get_where('download',array('id'=>$param))->row('pdf');
        	    if(!empty($row_data)) {
	        ...
	        if(isset($update)){
                $message = array('message'=>"download Updated Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Update download", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/download'),'refresh');
    ...
    public function delete($param='')
	{
        ...
        $delete = $this->db->where('id', $param)->delete('download');
        
        if(isset($delete)){
            $message = array('message'=>"download Delete Successfully", 'class'=>'success');
        }else{
            $message = array('message'=>"Failed to Delete download", 'class'=>'danger');
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/download'),'refresh');
	}
	...
	public function fetch_data() {
        ...
        $main_table = 'download'; // Main table
        $controller_name = "admin/Download"; // Controller name
        ...
        $category_filter = $this->input->post('download_category');
        ...
        $orderable_columns = ["$main_table.id", null,'download_category.eng_title', "$main_table.eng_title", "$main_table.guj_title",null];
        ...
        $this->db->select("$main_table.*,download_category.eng_title as dc_eng_title")
                 ->from($main_table)
                 ->join('download_category', "$main_table.download_category_id = download_category.id", 'left');
                 
        // Apply the language filter if selected
        if (!empty($category_filter)) {
            $this->db->where('download_category.id', $category_filter);
        }
        ...
        if (!empty($search_value)) {
            $this->db->group_start() // Start the grouping of LIKE clauses
                     ...
                     ->or_like('download_category.eng_title', $search_value)
