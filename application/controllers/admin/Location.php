...
class Location extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('footer_info', 'Footer Info', 'trim');

        if ($this->form_validation->run() == false) {
            $message = array('message' => (validation_errors()), 'class' => 'danger');
            redirect(site_url('admin/location'), 'refresh');
        } else {
            ...
            $data['footer_info'] = $this->security->xss_clean($this->input->post('footer_info'));

            $insert = $this->db->insert('location', $data);
            if (isset($insert)) {
                $message = array('message' => "Location Created Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Create Location", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/location'), 'refresh');
    ...
    public function edit($param = '')
    {
        ...
        $page_data['page_name'] = 'location';
        $page_data['languages'] = $languages;
        $page_data['row_data'] = $this->db->get_where('location', array('id' => $param))->row_array();
        $this->load->view('admin/common', $page_data);
    }
    public function update($param = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($update)) {
                $message = array('message' => "Location Updated Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Update Location", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/location'), 'refresh');
    ...
    {
        $delete = $this->db->where('id', $param)->delete('location');

        if (isset($delete)) {
            $message = array('message' => "Location Delete Successfully", 'class' => 'success');
        } else {
            $message = array('message' => "Failed to Delete Location", 'class' => 'danger');
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/location'), 'refresh');
    }
    ...
    public function fetch_data()
    {
        ...
        $main_table = 'location'; // Main table
        $controller_name = "admin/Location"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "language.name", "$main_table.title", "$main_table.sequence", null, null, null];
        ...
        foreach ($data as $row) {
            ...
            $row_data[] = $row['footer_info'];
            $row_data[] = '<a href="' . $row['link'] . '" target="_blank"><i class="fa fa-eye" aria-hidden="true"></i></a>';
