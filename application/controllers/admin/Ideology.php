...
class Ideology extends CI_Controller {
	...
	public function index() {
	    ...
	    $page_data['page_name'] = "ideology";
        $page_data['page_title'] = "Ideology";
	...
	public function create(){
	   ...
	   // $this->form_validation->set_rules('photo', 'Photo', 'required');
        ...
        if($this->form_validation->run() == false){
            ...
            redirect(site_url('admin/ideology'),'refresh');
        }else{
            ...
            $insert = $this->db->insert('ideology',$data);          
            if(isset($insert)){
                $message = array('message'=>"Ideology Created Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Create Ideology", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/ideology'),'refresh');
	...
	public function edit($param=''){
        ...
        $page_data['page_name'] = 'ideology';
        $page_data['languages'] = $languages;
        $page_data['row_data']  =   $this->db->get_where('ideology',array('id'=>$param))->row_array();
        $this->load->view('admin/common', $page_data);
	}
	public function update($param=''){
        ...
        if($this->form_validation->run() == false){
            ...
            if(!empty($_FILES['photo']['name'])) {
                $row_data  =  $this->db->get_where('ideology',array('id'=>$param))->row('photo');
        	    if(!empty($row_data)) {
	        ...
	        $update = $this->db->update('ideology',$data);
	        if(isset($update)){
                $message = array('message'=>"Ideology Updated Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Update Ideology", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/ideology'),'refresh');
    ...
    public function delete($param='')
	{
        ...
        $delete = $this->db->where('id', $param)->delete('ideology');
        
        if(isset($delete)){
            $message = array('message'=>"Ideology Delete Successfully", 'class'=>'success');
        }else{
            $message = array('message'=>"Failed to Delete Ideology", 'class'=>'danger');
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/ideology'),'refresh');
	}
	...
	public function fetch_data() {
        ...
        $main_table = 'ideology'; // Main table
        $controller_name = "admin/Ideology"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "language.name", "$main_table.title","$main_table.description",null];
