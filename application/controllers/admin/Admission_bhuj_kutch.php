...
class Admission_bhuj_kutch extends CI_Controller {
	...
	public function index() {
	    ...
	    $page_data['page_name'] = "admission_bhuj_kutch";
        $page_data['page_title'] = "Bhuj Kutch Admission";
	...
	public function create(){
	   ...
	   // $this->form_validation->set_rules('photo', 'Photo', 'required');
        ...
        if($this->form_validation->run() == false){
            ...
            redirect(site_url('admin/admission_bhuj_kutch'),'refresh');
        }else{
            ...
            $insert = $this->db->insert('admission_bhuj_kutch',$data);          
            if(isset($insert)){
                $message = array('message'=>"Admission Created Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Create Admission", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/admission_bhuj_kutch'),'refresh');
	...
	public function edit($param=''){
        ...
        $page_data['page_name'] = 'admission_bhuj_kutch';
        $page_data['languages'] = $languages;
        $page_data['row_data']  =   $this->db->get_where('admission_bhuj_kutch',array('id'=>$param))->row_array();
        $this->load->view('admin/common', $page_data);
	}
	public function update($param=''){
        ...
        if($this->form_validation->run() == false){
	        ...
	        $update = $this->db->update('admission_bhuj_kutch',$data);
	        if(isset($update)){
                $message = array('message'=>"Admission Updated Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Update Admission", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/admission_bhuj_kutch'),'refresh');
    ...
    public function delete($param='')
	{
        ...
        $delete = $this->db->where('id', $param)->delete('admission_bhuj_kutch');
        
        if(isset($delete)){
            $message = array('message'=>"Admission Delete Successfully", 'class'=>'success');
        }else{
            $message = array('message'=>"Failed to Delete Admission", 'class'=>'danger');
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/admission_bhuj_kutch'),'refresh');
	}
	...
	public function fetch_data() {
        ...
        $main_table = 'admission_bhuj_kutch'; // Main table
        $controller_name = "admin/Admission_bhuj_kutch"; // Controller name
