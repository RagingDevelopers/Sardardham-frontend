...
class President_message extends CI_Controller {
	...
	public function index() {
	    ...
	    $page_data['page_name'] = "president_message";
        $page_data['page_title'] = "President Message";
	...
	public function create(){
	   ...
	   // $this->form_validation->set_rules('photo', 'Photo', 'required');
        ...
        if($this->form_validation->run() == false){
            ...
            redirect(site_url('admin/president_message'),'refresh');
        }else{
            $data['language_id'] = $this->security->xss_clean($this->input->post('language_id'));
            $data['main_title'] = $this->security->xss_clean($this->input->post('main_title'));
            ...
            $insert = $this->db->insert('president_message',$data);          
            if(isset($insert)){
                $message = array('message'=>"Message Created Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Create Message", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/president_message'),'refresh');
	...
	public function edit($param=''){
        ...
        $page_data['page_name'] = 'president_message';
        $page_data['languages'] = $languages;
        $page_data['row_data']  =   $this->db->get_where('president_message',array('id'=>$param))->row_array();
        $this->load->view('admin/common', $page_data);
	}
	public function update($param=''){
        ...
        if($this->form_validation->run() == false){
            ...
            if(!empty($_FILES['photo']['name'])) {
                $row_data  =  $this->db->get_where('president_message',array('id'=>$param))->row('photo');
        	    if(!empty($row_data)) {
	        ...
	        $update = $this->db->update('president_message',$data);
	        if(isset($update)){
                $message = array('message'=>"Message Updated Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Update Message", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/president_message'),'refresh');
    ...
    public function delete($param='')
	{
        ...
        $delete = $this->db->where('id', $param)->delete('president_message');
        
        if(isset($delete)){
            $message = array('message'=>"Message Delete Successfully", 'class'=>'success');
        }else{
            $message = array('message'=>"Failed to Delete Message", 'class'=>'danger');
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/president_message'),'refresh');
	}
	...
	public function fetch_data() {
        ...
        $main_table = 'president_message'; // Main table
        $controller_name = "admin/President_message"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "language.name", "$main_table.main_title","$main_table.sub_title","$main_table.description",null];
        ...
        if (!empty($search_value)) {
            $this->db->group_start() // Start the grouping of LIKE clauses
                     ->like("$main_table.main_title", $search_value)
