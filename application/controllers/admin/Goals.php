...
class Goals extends CI_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $insert = $this->db->insert('goals', $data);
            if (isset($insert)) {
                $message = array('message' => "Goals Created Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Create Goals", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/goals'), 'refresh');
    ...
    public function edit($param = '')
    {
        ...
        $page_data['page_name'] = 'goals';
        $page_data['languages'] = $languages;
        $page_data['row_data'] = $this->db->get_where('goals', array('id' => $param))->row_array();
        $this->load->view('admin/common', $page_data);
    }
    public function update($param = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // if (!empty($_FILES['photo']['name'])) {
            //     $row_data = $this->db->get_where('goals', array('id' => $param))->row('photo');
            //     if (!empty($row_data)) {
            ...
            if (!empty($_FILES['icon']['name'])) {
                $row_data = $this->db->get_where('goals', array('id' => $param))->row('icon');
                if (!empty($row_data) && is_file("./upload/" . $row_data)) {
                    unlink("./upload/" . $row_data);
                }
                $file_ext = pathinfo($_FILES['icon']['name'], PATHINFO_EXTENSION);
                $rand = substr(md5(microtime()), rand(0, 26), 3);
            ...
            $update = $this->db->update('goals', $data);
            if (isset($update)) {
                $message = array('message' => "Goals Updated Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Update Goals", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/goals'), 'refresh');
    ...
    public function delete($param = '')
    {
        ...
        $delete = $this->db->where('id', $param)->delete('goals');

        if (isset($delete)) {
            $message = array('message' => "Goals Delete Successfully", 'class' => 'success');
        } else {
            $message = array('message' => "Failed to Delete Goals", 'class' => 'danger');
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/goals'), 'refresh');
    }
    ...
    public function fetch_data()
    {
        ...
        $main_table = 'goals'; // Main table
        $controller_name = "admin/Goals"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "language.name", "$main_table.title", "$main_table.sequence", null, null];
        ...
        foreach ($data as $row) {
            ...
            $row_data[] = '<a href="' . base_url('upload/' . $row['icon']) . '" target="_blank"><i class="fa fa-eye" aria-hidden="true"></i></a>';
