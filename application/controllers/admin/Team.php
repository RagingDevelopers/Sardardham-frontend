...
class Team extends CI_Controller
{
    ...
    function create()
    {
        ...
        $this->form_validation->set_rules('eng_designation', 'English Designation', 'trim|required');
        $this->form_validation->set_rules('guj_designation', 'Gujarati Designation', 'trim|required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($insert)) {
                $message = array('message' => "team Created Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Create team", 'class' => 'danger');
            }
        }
    ...
    public function edit($param = '')
    {
        ...
        $page_data['page_title'] = 'team';
        $page_data['page_name'] = 'team';
        $page_data['row_data'] = $this->db->get_where('team', array('id' => $param))->row_array();
        $page_data['data'] = $this->db->get('team')->result_array();
        $this->load->view('admin/common', $page_data);
    ...
    public function update($param = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $update = $this->db->update('team', $data);
            if (isset($update)) {
                $message = array('message' => "team Updated Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Update team", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/team'), 'refresh');
    ...
    public function delete($param = '')
    {
        ...
        $delete = $this->db->where('id', $param)->delete('team');

        if (isset($delete)) {
            $message = array('message' => "team Delete Successfully", 'class' => 'success');
        } else {
            $message = array('message' => "Failed to Delete team", 'class' => 'danger');
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/team'), 'refresh');
    }
    ...
    public function fetch_data()
    {
        ...
        $main_table = 'team'; // Main table
        $controller_name = "admin/Team"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "$main_table.eng_name", "$main_table.guj_name", "$main_table.eng_designation", "$main_table.guj_designation", "$main_table.sequence", null];
