...
class Download_category extends CI_Controller {
	...
	public function index() {
	    $page_data['page_name'] = "download_category";
        $page_data['page_title'] = "Download Category";
	...
	public function create(){
        ...
        if($this->form_validation->run() == false){
            ...
            $insert = $this->db->insert('download_category',$data);          
            if(isset($insert)){
                $message = array('message'=>"Download Category Created Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Create Download Category", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/download_category'),'refresh');
	}
	public function edit($param=''){
        $param=$this->security->xss_clean($param);
        $page_data['page_title'] = 'Edit Download Category';
        $page_data['page_name'] = 'download_category';
        $page_data['row_data'] = $this->db->get_where('download_category', array('id' => $param))->row_array();
	...
	public function update($param=''){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $update = $this->db->update('download_category', $data);
            
            if ($update) {
                $message = array('message' => "Download Category Updated Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Update Download Category", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/download_category'), 'refresh');
	...
	{
	    $delete = $this->db->where('id', $param)->delete('download_category');

        if ($delete) {
            $message = array('message' => "Download Category Deleted Successfully", 'class' => 'success');
        } else {
            $message = array('message' => "Failed to Delete Download Category", 'class' => 'danger');
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/download_category'), 'refresh');
	}
	...
	public function fetch_data() {
        ...
        $main_table = 'download_category'; // Main table
        $controller_name = "admin/Download_category"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "$main_table.eng_title", "$main_table.guj_title","$main_table.created_at"];
