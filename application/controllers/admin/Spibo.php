...
class Spibo extends CI_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(site_url('admin/spibo'), 'refresh');
        } else {
            ...
            $insert = $this->db->insert('spibo', $data);
            if (isset($insert)) {
                $message = array('message' => "SPIBO Created Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Create SPIBO", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/spibo'), 'refresh');
    ...
    public function edit($param = '')
    {
        ...
        $page_data['page_name'] = 'spibo';
        $page_data['languages'] = $languages;
        $page_data['row_data'] = $this->db->get_where('spibo', array('id' => $param))->row_array();
        $this->load->view('admin/common', $page_data);
    }
    public function update($param = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $update = $this->db->update('spibo', $data);
            if (isset($update)) {
                $message = array('message' => "SPIBO Updated Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Update SPIBO", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/spibo'), 'refresh');
    ...
    public function delete($param = '')
    {
        ...
        $delete = $this->db->where('id', $param)->delete('spibo');

        if (isset($delete)) {
            $message = array('message' => "SPIBO Delete Successfully", 'class' => 'success');
        } else {
            $message = array('message' => "Failed to Delete SPIBO", 'class' => 'danger');
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/spibo'), 'refresh');
    }
    ...
    public function fetch_data()
    {
        ...
        $main_table = 'spibo'; // Main table
        $controller_name = "admin/Spibo"; // Controller name
