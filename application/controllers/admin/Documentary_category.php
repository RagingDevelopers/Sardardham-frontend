...
class Documentary_category extends CI_Controller
{
    ...
    {
        $page_data['page_name'] = "documentary_category";
        $page_data['page_title'] = "Documentary Category";
    ...
    public function create()
    {
        $this->form_validation->set_rules('eng_name', 'English Name', 'trim|required')
            ->set_rules('guj_name', 'Gujarati Name', 'trim|required')
            ->set_rules('eng_year', 'English Year', 'trim|required')
            ->set_rules('guj_year', 'Gujarati Year', 'trim|required');

        if ($this->form_validation->run() == false) {
            $message = array('message' => (validation_errors()), 'class' => 'danger');
        } else {
            $data['eng_name'] = $this->security->xss_clean($this->input->post('eng_name'));
            ...
            $insert = $this->db->insert('documentary_category', $data);
            if (isset($insert)) {
                $message = array('message' => "Documentary Category Created Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Create Documentary Category", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/documentary_category'), 'refresh');
    }
    public function edit($param = '')
    {
        $param = $this->security->xss_clean($param);
        $page_data['page_title'] = 'Edit Documentary Category';
        $page_data['page_name'] = 'documentary_category';
        $page_data['row_data'] = $this->db->get_where('documentary_category', array('id' => $param))->row_array();
    ...
    public function update($param = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $update = $this->db->update('documentary_category', $data);

            if ($update) {
                $message = array('message' => "Documentary Category Updated Successfully", 'class' => 'success');
            } else {
                $message = array('message' => "Failed to Update Documentary Category", 'class' => 'danger');
            }
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/documentary_category'), 'refresh');
    ...
    {
        $delete = $this->db->where('id', $param)->delete('documentary_category');

        if ($delete) {
            $message = array('message' => "Documentary Category Deleted Successfully", 'class' => 'success');
        } else {
            $message = array('message' => "Failed to Delete Documentary Category", 'class' => 'danger');
        }
        $this->session->set_flashdata('flash_message', $message);
        redirect(site_url('admin/documentary_category'), 'refresh');
    }
    ...
    public function fetch_data()
    {
        ...
        $main_table = 'documentary_category'; // Main table
        $controller_name = "admin/documentary_category"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "$main_table.eng_name", "$main_table.guj_name", "$main_table.eng_year", "$main_table.guj_year", "$main_table.created_at"];
        ...
        if (!empty($search_value)) {
            $this->db->group_start() // Start the grouping of LIKE clauses
                ->like("$main_table.eng_name", $search_value)
                ->or_like('documentary.guj_name', $search_value)
        ...
        if (!empty($search_value)) {
            $this->db->group_start()
                ->like("$main_table.eng_name", $search_value)
                ->or_like("$main_table.guj_name", $search_value)
                ->group_end();
        }
