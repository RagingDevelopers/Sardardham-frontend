...
class Mission_vision extends CI_Controller {
	...
	public function index() {
	    ...
	    $page_data['page_name'] = "mission_vision";
        $page_data['page_title'] = "Mission Vision Goals";
	...
	public function create(){
	   ...
	   // $this->form_validation->set_rules('photo', 'Photo', 'required');
        ...
        if($this->form_validation->run() == false){
            ...
            redirect(site_url('admin/mission_vision'),'refresh');
        }else{
            ...
            $insert = $this->db->insert('mission_vision',$data);          
            if(isset($insert)){
                $message = array('message'=>"Mission Vision Goals Created Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Create Mission Vision Goals", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/mission_vision'),'refresh');
	...
	public function edit($param=''){
        ...
        $page_data['page_name'] = 'mission_vision';
        $page_data['languages'] = $languages;
        $page_data['row_data']  =   $this->db->get_where('mission_vision',array('id'=>$param))->row_array();
        $this->load->view('admin/common', $page_data);
	}
	public function update($param=''){
        ...
        if($this->form_validation->run() == false){
	        ...
	        $update = $this->db->update('mission_vision',$data);
	        if(isset($update)){
                $message = array('message'=>"Mission Vision Goals Updated Successfully", 'class'=>'success');
            }else{
                $message = array('message'=>"Failed to Update Mission Vision Goals", 'class'=>'danger');
            }
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/mission_vision'),'refresh');
    ...
    public function delete($param='')
	{
        ...
        $delete = $this->db->where('id', $param)->delete('mission_vision');
        
        if(isset($delete)){
            $message = array('message'=>"Mission Vision Goals Delete Successfully", 'class'=>'success');
        }else{
            $message = array('message'=>"Failed to Delete Mission Vision Goals", 'class'=>'danger');
        }
        $this->session->set_flashdata('flash_message',$message);
        redirect(site_url('admin/mission_vision'),'refresh');
	}
	...
	public function fetch_data() {
        ...
        $main_table = 'mission_vision'; // Main table
        $controller_name = "admin/Mission_vision"; // Controller name
        ...
        $orderable_columns = ["$main_table.id", null, "language.name", "$main_table.type","$main_table.description"];
        ...
        if (!empty($search_value)) {
            $this->db->group_start() // Start the grouping of LIKE clauses
                     ->like("$main_table.type", $search_value)
